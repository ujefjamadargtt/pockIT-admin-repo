...
@Component({
  ...
  styleUrls: ['./job-wise-tickets-raisedreport.component.css']
})
export class JobWiseTicketsRaisedreportComponent {
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (this.TechnicianNameText !== '') {
      likeQuery +=
        (likeQuery ? ' AND ' : '') +
        `CATEGORY_NAME LIKE '%${this.TechnicianNameText.trim()}%'`;
      this.isTechnicianNameFilterApplied = true;
    } else {
      this.isTechnicianNameFilterApplied = false;
    }
    ...
    if (this.TerritoryNameText !== '') {
      likeQuery +=
        (likeQuery ? ' AND ' : '') +
        `SUB_CATEGORY_NAME LIKE '%${this.TerritoryNameText.trim()}%'`;
      this.isTerritoryNameFilterApplied = true;
    } else {
      this.isJobCardNoFilterApplied = false;
    }
  ...
  convertInExcel() {
    ...
    for (var i = 0; i < this.branchData.length; i++) {
      ...
      obj1['Raised Datetime'] = this.branchData[i]['ORDER_DATE_TIME']
        ? this.datePipe.transform(
          this.branchData[i]['ORDER_DATE_TIME'],
          'dd/MM/yyyy'
        )
        : '-';
      obj1['Order No'] = this.branchData[i]['ORDER_NO'];
      obj1['Qualification Datetime'] = this.branchData[i]['ORDER_ACCEPTED_DATE']
        ? this.datePipe.transform(
          this.branchData[i]['ORDER_ACCEPTED_DATE'],
          'dd/MM/yyyy'
        )
        : '-';
      obj1['Technician Assigned Datetime'] = this.branchData[i]['SHEDULED_DATETIME']
        ? this.datePipe.transform(
          this.branchData[i]['SHEDULED_DATETIME'],
          'dd/MM/yyyy'
        )
        : '-';
      obj1['Closer Datetime'] = this.branchData[i]['JOB_COMPLETED_DATETIME']
        ? this.datePipe.transform(
          this.branchData[i]['JOB_COMPLETED_DATETIME'],
          'dd/MM/yyyy'
        )
        : '-';
