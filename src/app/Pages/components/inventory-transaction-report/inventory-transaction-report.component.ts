...
export class InventoryTransactionReportComponent {
  ...
  onitemChange(): void {
    if (this.selecteditems?.length) {
      this.search();
      this.isitemFilterApplied = true; // Filter applied if selectedCategories has values
    } else {
      this.search();
      this.isitemFilterApplied = false; // Filter reset if selectedCategories is null, undefined, or empty
    }
  }
  ...
  onadjustChange(): void {
    if (this.selectedadjust?.length) {
      this.search();
      this.isadjustFilterApplied = true; // Filter applied if selectedCategories has values
    } else {
      this.search();
      this.isadjustFilterApplied = false; // Filter reset if selectedCategories is null, undefined, or empty
    }
  }
  ...
  oninwardChange(): void {
    if (this.selectedinwards?.length) {
      this.search();
      this.isinwardFilterApplied = true; // Filter applied if selectedCategories has values
    } else {
      this.search();
      this.isinwardFilterApplied = false; // Filter reset if selectedCategories is null, undefined, or empty
    }
  }
  distinctData: any = [];
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (this.selectedCountries.length > 0) {
      ...
      likeQuery += `MOVEMENT_NAME IN ('${this.selectedCountries.join("','")}')`; // Update with actual field name in the DB
    }
    ...
    if (this.selecteditems.length > 0) {
      ...
      likeQuery += `ITEM_NAME IN ('${this.selecteditems.join("','")}')`; // Update with actual field name in the DB
    }
    ...
    if (this.selectedware.length > 0) {
      ...
      likeQuery += `WAREHOUSE_NAME IN ('${this.selectedware.join("','")}')`; // Update with actual field name in the DB
    }
    ...
    if (this.selectedadjust.length > 0) {
      ...
      likeQuery += `ADJUSTMENT_NAME IN ('${this.selectedadjust.join("','")}')`; // Update with actual field name in the DB
    }
    ...
    if (this.selectedinwards.length > 0) {
      ...
      likeQuery += `INWARD_NAME IN ('${this.selectedinwards.join("','")}')`; // Update with actual field name in the DB
    }
