...
export class WhatsappServiceConfigComponent {
  ...
  save(addNew: boolean, whatsappDrawer: NgForm): void {
    ...
    if (
      ...
      (this.data.PHONE_NUMBER_ID == null ||
        this.data.PHONE_NUMBER_ID == undefined ||
        this.data.PHONE_NUMBER_ID.trim() == '') &&
      ...
      this.isOk = false;
      this.message.error('Please enter the Authentication Type.', '');
    }
    if (this.isOk && !this.data.API_URL?.trim()) {
      this.isOk = false;
      this.message.error('Please enter the API URL.', '');
    } else if (this.isOk && !/^https?:\/\/.+$/.test(this.data.API_URL)) {
      this.isOk = false;
      this.message.error('Please enter a valid API URL.', '');
    }
    if (this.isOk && !this.data.API_KEY?.trim()) {
      this.isOk = false;
      this.message.error('Please enter the API Key.', '');
    } else if (!this.TIMEOUT_DATE) {
      this.isOk = false;
    ...
    if (
      ...
      this.isOk = false;
      this.message.error('Please enter the Phone Number ID.', '');
    } else if (
      this.data.DEFAULT_COUNTRY_CODE == null ||
      this.data.DEFAULT_COUNTRY_CODE == undefined ||
      this.data.DEFAULT_COUNTRY_CODE.trim() == ''
      ...
      this.isOk = false;
      this.message.error('Please enter the Default Country Code.', '');
    } else if (this.TIMEOUT_DATE == null || this.TIMEOUT_DATE == undefined) {
      this.isOk = false;
      this.message.error('Please provide the Timeout Date.', '');
    }
    ...
    if (this.isOk) {
      ...
      if (this.data.ID) {
          ...
          (response: any) => {
            ...
            if (response.code === 200) {
              this.message.success(
                'Whatsapp Service configuration Data Updated Successfully.',
                ''
              );
              if (!addNew) this.drawerClose();
            } else {
              this.message.error(
                'Failed to update Whatsapp Service configuration Data.',
                ''
              );
              this.isSpinning = false;
            }
        ...
        this.api.CreateWhatsappServiceConfigData(this.data).subscribe(
          (response: any) => {
            this.isSpinning = false;
            if (response.code === 200) {
              this.message.success(
                'Whatsapp Service configuration Data Created Successfully.',
                ''
              );
              if (!addNew) this.drawerClose();
              else {
                this.resetDrawer(whatsappDrawer);
                this.api
                  .getWhatsappServiceConfigData(0, 0, '', 'desc', '')
                  .subscribe(
                    () => { },
                    () => { }
                  );
              ...
              this.message.error(
                'Failed to create Whatsapp Service configuration Data.',
                ''
              );
              this.isSpinning = false;
            }
