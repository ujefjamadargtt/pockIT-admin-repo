...
@Component({
  ...
  styleUrls: ['./vendor-territory-mapping.component.css'],
})
export class VendorTerritoryMappingComponent {
  @Input() data;
  @Input() mappedterritory;
  @Input() ROLE_ID;
  ...
  getTeritoryByCountry(countryId: any) {
    ...
    var terFilt = '';
    if (this.ROLE_ID === 5) {
      terFilt = " AND ID IN(" + this.mappedterritory + ")"
    } else {
      terFilt = ''
    }
    this.territoryloading = true;
    this.api
      .getTeritory(0, 0, '', 'asc', ' AND IS_ACTIVE =1 AND COUNTRY_ID = ' + countryId + terFilt)
      .subscribe(
        (data) => {
          if (data['code'] === 200) {
      ...
      this.api.mapVendorTerritoryMapping(this.data.ID, this.data.USER_ID, this.data.NAME, 1, dataToSave).subscribe(
        (successCode) => {
          ...
          if (successCode['code'] === 200) {
            ...
            this.message.error('Failed to map task to the vendor.', '');
          }
          this.isSpinning = false;
        },
        () => {
