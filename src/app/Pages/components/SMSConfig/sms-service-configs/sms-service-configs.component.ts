...
export class SmsServiceConfigsComponent {
  formTitle = 'Manage SMS Service Configuration';
  ...
  search(reset: boolean = false) {
    ...
    if (this.authenticiation && this.authenticiation.length > 0) {
      const authConditions = this.authenticiation
        .map((type) => `AUTHENTICATION_TYPE LIKE '%${type}%'`)
        .join(' OR ');
      ...
      .subscribe(
        (data) => {
          if (data['code'] == 200) {
            ...
            this.smsServiceConfigData = data['data'];
          } else if (data['code'] == 400) {
            this.loadingRecords = false;
            this.smsServiceConfigData = [];
  ...
  setFilterApplied(field: string, value: boolean): void {
    switch (field) {
      case 'ProviderText':
        this.isProviderTextApplied = value;
        break;
      case 'URLText':
        this.isURLTextApplied = value;
        break;
      case 'KeyText':
        this.isKeyTextApplied = value;
        break;
      case 'SenderText':
        this.isSenderTextApplied = value;
        break;
      case 'UserText':
        this.isUserTextApplied = value;
        break;
      default:
        break;
    }
  ...
  reset(): void {
    ...
    this.isKeyTextApplied = false;
    this.isSenderTextApplied = false;
    this.isUserTextApplied = false;
    this.isAuthenticiationFilterApplied = false;
