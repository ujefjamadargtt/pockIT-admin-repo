...
export class BackOfficeMasterDrawerComponent {
  ...
  ngOnInit(): void {
    if (
      ...
      this.fullImageUrl =
        this.retriveimgUrl + "BackofficeProfile/" + this.data.PROFILE_PHOTO;
      ...
      .subscribe(
        (data) => {
          if (data["code"] == 200) {
            ...
            var vendorroleid = this.commonFunction.decryptdata(
              sessionStorage.getItem("roleId") || ""
            );
            if (vendorroleid == "5") {
              this.roleData = this.roleData.filter((data) => {
  ...
  reportingid: any;
  backofficedata: any[] = [];
  getreportingperson(roleId: any, value: boolean) {
    this.backoffice = true;
    this.reportingid = roleId;
    this.api.getBackOfficeData(0, 0, "", "asc", " AND ROLE_ID = 24").subscribe(
      (data) => {
        if (data["code"] === 200) {
          ...
          this.backofficedata = [];
          this.message.error("Failed To Get Backoffice Data...", "");
          this.backoffice = false;
        }
      },
      () => {
  ...
  save(addNew: boolean, BackOfficeForm: NgForm): void {
    ...
    if (
      ...
      this.isOk = false;
      this.message.error("Please enter the team member name.", "");
    } else if (
      this.data.EMAIL_ID == null ||
      this.data.EMAIL_ID == undefined ||
      this.data.EMAIL_ID.trim() == ""
      ...
      this.data.EMAIL_ID != undefined &&
      this.data.EMAIL_ID.trim() != "" &&
      !this.emailPattern.test(this.data.EMAIL_ID.trim())
    ) {
      this.isOk = false;
      ...
      this.isOk = false;
      this.message.error("Please enter a mobile number.", "");
    } else if (
      this.data.MOBILE_NUMBER != null &&
      this.data.MOBILE_NUMBER != undefined &&
      this.data.MOBILE_NUMBER != 0 &&
    ...
    if (this.data.ID) {
        ...
        (successCode: any) => {
          if (successCode.code == 200) {
            this.message.success(
              "Back office team member updated successfully.",
              ""
            );
            ...
            this.message.error("Back office team member updation failed.", "");
            this.isSpinning = false;
          }
        },
        (err) => {
      ...
      this.api.createBackOffice(this.data).subscribe(
        (successCode: any) => {
          if (successCode.code == 200) {
            this.message.success(
              "Back office team member created successfully.",
              ""
            );
            if (!addNew) {
              this.drawerClose();
            ...
            this.message.error("Back office team member creation failed.", "");
          }
          this.isSpinning = false;
        },
        (error) => {
