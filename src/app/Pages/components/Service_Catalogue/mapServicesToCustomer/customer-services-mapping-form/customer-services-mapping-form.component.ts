...
@Component({
  selector: "app-customer-services-mapping-form",
  templateUrl: "./customer-services-mapping-form.component.html",
  styleUrls: ["./customer-services-mapping-form.component.css"],
  providers: [DatePipe],
})
export class CustomerServicesMappingFormComponent implements OnInit {
  uniteDta: any = [];
  taxData: any = [];
  isFocused: string = "";
  isSpinning = false;
  ...
  currencyData: any = [];
  // hours: any='00';
  @Input() data: any = ServiceCatMasterDataNewNonB2b;
  @Input() terittorydata: any = customer;
  ...
  resetDrawer(ServiceCatmaster: NgForm) {
    this.data = new ServiceCatMasterDataNewNonB2b();
  ...
  save(addNew: boolean, ServiceCatmaster: NgForm): void {
    ...
    // ) {
    //   this.isOk = false;
    //   this.message.error(" Please enter service cost for B2C (₹)", "");
    // } else
    if (
      this.data.B2B_PRICE === null ||
    ...
    if (this.isOk) {
...

      {
        var datalist: any = {
          ...
          B2C_PRICE: 0,
          EXPRESS_COST: this.data.IS_EXPRESS ? this.data.EXPRESS_COST : 0,
          ...
          CUSTOMER_ID: this.terittorydata.ID,
          ...
          IS_EXPRESS: this.data.IS_EXPRESS ? true : false,
          NAME: this.data.NAME,
          DESCRIPTION: this.data.DESCRIPTION,
          SERVICE_TYPE: "B",
          PREPARATION_MINUTES: this.data.PREPARATION_MINUTES,
          ...
          SUB_CATEGORY_ID: this.servicemaindata.SUB_CATEGORY_ID
            ? this.servicemaindata.SUB_CATEGORY_ID
            : 0,
          DURARTION_HOUR: this.servicemaindata.DURARTION_HOUR
            ? this.servicemaindata.DURARTION_HOUR
            : 0,
          DURARTION_MIN: this.servicemaindata.DURARTION_MIN
            ? this.servicemaindata.DURARTION_MIN
            : 0,
          ...
          IS_JOB_CREATED_DIRECTLY: this.servicemaindata.IS_JOB_CREATED_DIRECTLY
            ...
            : false,
          ORG_ID: 1,
          QTY: this.servicemaindata.QTY ? this.servicemaindata.QTY : 1,
        ...
        if (this.editclick == "Y") {
          this.api.getServiceTerritoryDetailsupdateb2b(datalist).subscribe(
            (successCode: any) => {
          ...
          this.api.getServiceTerritoryDetailscreateb2b(datalist).subscribe(
            (successCode: any) => {
              if (successCode.code == "200") {
