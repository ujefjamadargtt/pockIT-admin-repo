...
import {
  ServiceCatMasterDataNewB2b,
  ServiceCatMasterDataNewNonB2b,
} from 'src/app/Pages/Models/ServiceCatMasterData';
import { ApiServiceService } from 'src/app/Service/api-service.service';
import { CommonFunctionService } from 'src/app/Service/CommonFunctionService';
import { forkJoin } from 'rxjs';
@Component({
  selector: 'app-customer-services-mapping-list',
  templateUrl: './customer-services-mapping-list.component.html',
  styleUrls: ['./customer-services-mapping-list.component.css'],
  providers: [DatePipe],
})
export class CustomerServicesMappingListComponent implements OnInit {
  drawerVisible: boolean = false;
  drawerData: ServiceCatMasterDataNewNonB2b =
  ...
  getServiceTerritoryget() {
    this.isSpinningMulti = true;
    this.api.getServiceCustomerget(0, 0, '', '', '', this.data.ID).subscribe(
      (data) => {
        if (data['code'] == 200) {
          this.isSpinningMulti = false;
          if (data['count'] > 0) {
  ...
  search(reset: boolean = false) {
    ...
    this.api
      .getServiceTerritoryNongetB2B(
        ...
        '',
        sort,
        likeQuery + ' AND CUSTOMER_ID=' + this.data.ID
      )
      .subscribe(
        (data) => {
          if (data['code'] == 200) {
  ...
  saveserviceMulti() {
    if (
      ...
      this.loadingRecords = true;
      this.api
        .b2bAvailabilityMapping(this.data.ID, this.Service_HEI_DATA)
        .subscribe(
          (successCode: any) => {
  ...
  sername: any;
  editclick: any = 'N';
  edit(data: ServiceCatMasterDataNewNonB2b): void {
    ...
    if (!selection1) {
      ...
      this.api
        .getServiceTerritoryNongetB2B(
          ...
          '',
          sort,
          newQuery + ' AND CUSTOMER_ID=' + this.data.ID
        )
        .subscribe(
          (data) => {
            if (data['code'] === 200) {
    ...
    if (!selection1) {
      ...
      this.api
        .getServiceTerritoryNongetB2B(
          ...
          '',
          sort,
          FILDATA + ' AND CUSTOMER_ID=' + this.data.ID
        )
        .subscribe(
          (data) => {
            if (data['code'] === 200) {
    ...
    if (this.updatedRecords.length > 0) {
      ...
      this.updatedRecords.forEach((data, i) => {
        if (
          ...
          commonErrorMessage =
            'Service preparation time must be greater than 0';
          isValid = false;
        }
      });
      ...
      if (isValid) {
        ...
        forkJoin(requests).subscribe(responses => {
          responses.forEach((datas, index) => {
            ...
            if (datas['code'] == 200) {
              ...
              rowData.DURATION_HOUR = serviceData?.DURATION_HOUR
                ...
                : 0;
              rowData.DURATION_MIN = serviceData?.DURATION_MIN
                ? serviceData?.DURATION_MIN
                : 0;
              rowData.SHORT_CODE = serviceData?.SHORT_CODE
                ? serviceData?.SHORT_CODE
                : 0;
              rowData.MAX_QTY = serviceData?.MAX_QTY
                ? serviceData?.MAX_QTY
                : 0;
              rowData.IS_NEW = serviceData?.IS_NEW
                ? serviceData?.IS_NEW
                : false;
              rowData.PARENT_ID = serviceData?.PARENT_ID
                ? serviceData?.PARENT_ID
                : 0;
              rowData.IS_FOR_B2B = true;
              rowData.IS_JOB_CREATED_DIRECTLY = serviceData?.IS_JOB_CREATED_DIRECTLY
                ? serviceData?.IS_JOB_CREATED_DIRECTLY
                : false;
              rowData.ORG_ID = 1;
              rowData.QTY = serviceData?.QTY ? serviceData?.QTY : 0;
              rowData.STATUS = serviceData?.STATUS
                ? serviceData?.STATUS
                : false;
            }
          ...
          this.api
            .BulkServiceUpdateB2b(this.data.ID, this.updatedRecords)
            .subscribe(
              (successCode: any) => {
                if (successCode.code == '200') {
        ...
        .subscribe(
          (datas) => {
            if (datas['code'] == 200) {
              ...
              rowData.TERRITORY_ID = 0;
              rowData.DURATION_HOUR = datas['data'][0].DURATION_HOUR
                ? datas['data'][0].DURATION_HOUR
                : 0;
              rowData.DURATION_MIN = datas['data'][0].DURATION_MIN
                ? datas['data'][0].DURATION_MIN
                : 0;
              rowData.HSN_CODE = datas['data'][0].HSN_CODE
                ? datas['data'][0].HSN_CODE
                : null;
              rowData.HSN_CODE_ID = datas['data'][0].HSN_CODE_ID
                ? datas['data'][0].HSN_CODE_ID
                : null;
              ...
              rowData.SHORT_CODE = datas['data'][0].SHORT_CODE
                ...
                : 0;
              rowData.MAX_QTY = datas['data'][0].MAX_QTY
                ? datas['data'][0].MAX_QTY
                : 0;
              rowData.TAX_ID = datas['data'][0].TAX_ID
                ? datas['data'][0].TAX_ID
                : 0;
              rowData.IS_NEW = datas['data'][0].IS_NEW
                ? datas['data'][0].IS_NEW
                : false;
              rowData.PARENT_ID = datas['data'][0].PARENT_ID
                ? datas['data'][0].PARENT_ID
                : 0;
              rowData.IS_PARENT = datas['data'][0].IS_PARENT
                ? datas['data'][0].IS_PARENT
                : false;
              rowData.IS_FOR_B2B = true;
              rowData.SERVICE_TYPE = 'B';
              rowData.B2C_PRICE = null;
              rowData.IS_JOB_CREATED_DIRECTLY = datas['data'][0]
                .IS_JOB_CREATED_DIRECTLY
                ? datas['data'][0].IS_JOB_CREATED_DIRECTLY
                : false;
              rowData.ORG_ID = 1;
              rowData.QTY = datas['data'][0].QTY ? datas['data'][0].QTY : 0;
              rowData.STATUS = datas['data'][0].STATUS
                ? datas['data'][0].STATUS
                : false;
              this.isSpinning = true;
              this.api.BulkServiceUpdateB2b(this.data.ID, [rowData]).subscribe(
                (successCode: any) => {
                  if (successCode.code == '200') {
  ...
  drawerDataMains: ServiceCatMasterDataNewB2b =
    new ServiceCatMasterDataNewB2b();
  addMainService(): void {
    this.drawerTitleMains = 'Add New Service';
    this.dataList = [];
    this.pageIndex = 1;
    this.searchText = '';
