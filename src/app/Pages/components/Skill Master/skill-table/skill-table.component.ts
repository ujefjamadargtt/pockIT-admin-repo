...
export class SkillTableComponent {
  ...
  isSkillNameFilterApplied = false;
  ...
  onKeyup(event: KeyboardEvent): void {
    ...
    if (this.skilltext.length >= 3 && event.key === 'Enter') {
      this.search();
    } else if (this.skilltext.length == 0 && event.key === 'Backspace') {
      this.search();
    }

    if (this.skilldesctext.length >= 3 && event.key === 'Enter') {
      this.search();
    } else if (this.skilldesctext.length == 0 && event.key === 'Backspace') {
      this.search();
    }
  }
  ...
  search(reset: boolean = false) {
    ...
    if (this.skilltext !== '') {
      likeQuery +=
        (likeQuery ? ' AND ' : '') + `NAME LIKE '%${this.skilltext.trim()}%'`;
      this.isSkillNameFilterApplied = true;
    } else {
      this.isSkillNameFilterApplied = false;
    }

    if (this.skilldesctext !== '') {
      likeQuery +=
        (likeQuery ? ' AND ' : '') +
        `DESCRIPTION LIKE '%${this.skilldesctext.trim()}%'`;
