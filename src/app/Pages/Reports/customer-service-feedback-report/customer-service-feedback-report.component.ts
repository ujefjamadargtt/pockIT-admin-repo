...
@Component({
  ...
  styleUrls: ['./customer-service-feedback-report.component.css'],
})
export class CustomerServiceFeedbackReportComponent {
  drawerVisible: boolean = false;
  searchText: string = '';
  formTitle = 'Customer Service Feedback Report';
  pageIndex = 1;
  pageSize = 10;
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (this.servicename !== '') {
      likeQuery +=
        (likeQuery ? ' AND ' : '') +
        `SERVICE_NAME LIKE '%${this.servicename.trim()}%'`;
      this.isserviceNameFilterApplied = true;
    } else {
      this.isserviceNameFilterApplied = false;
    }
