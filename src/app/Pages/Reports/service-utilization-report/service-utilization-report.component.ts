...
export class ServiceUtilizationReportComponent {
  constructor(
  ...
  formTitle = 'Service Utilization Report';
  searchText: string = '';
  pageIndex = 1;
  pageSize = 10;
  sortKey: string = '';
  ...
  onKeyup(event: KeyboardEvent, type: any): void {
    if (this.nameText.length >= 3 && event.key === 'Enter' && type == 'name') {
      this.search();
  ...
  convertInExcel() {
    ...
    if (this.excelData.length > 0) {
      for (var i = 0; i < this.excelData.length; i++) {
        ...
        obj1['Total Requests'] = this.excelData[i]['TOTAL_REQUEST'];
        obj1['Completed Requests'] = this.excelData[i]['COMPLETED_REQUEST'];
