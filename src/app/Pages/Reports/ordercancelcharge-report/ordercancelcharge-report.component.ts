...
@Component({
  selector: 'app-ordercancelcharge-report',
  templateUrl: './ordercancelcharge-report.component.html',
  styleUrls: ['./ordercancelcharge-report.component.css']
})
export class OrdercancelchargeReportComponent {
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (this.paymentStatus === "unpaid") {
      paymentFilter =
        "  AND IFNULL(IS_CHARGES_PAID,0) <> 1";
    } else if (this.paymentStatus === "paid") {
      paymentFilter = " AND IS_CHARGES_PAID = 1";
    }
    ...
    if (exportInExcel == false) {
      this.api
        .Cancelorderget(
          ...
          this.sortKey,
          sort,
          likeQuery + this.filterQuery + " AND ORDER_STATUS  IN ('CA','PC') AND CANCELLATION_CHARGE >0 "+ paymentFilter + (filterQuery5 ? ' AND ' + filterQuery5 : '')
        )
        .subscribe(
          (data) => {
            if (data['code'] == 200) {
  ...
  convertInExcel() {
    ...
    if (this.excelData.length > 0) {
      for (var i = 0; i < this.excelData.length; i++) {
        ...
        obj1['Order Amount'] = this.excelData[i]['FINAL_AMOUNT'];
        obj1['Cancellation Charges'] = this.excelData[i]['CANCELLATION_CHARGE'];
        obj1['Cancellation Date'] = this.datepipe.transform(this.excelData[i]['CANCELLATION_DATE'], 'dd/MM/yyyy');
        obj1['Reason'] = this.excelData[i]['CANCELLATION_REASON'];
        obj1['Is Charges Paid'] = this.excelData[i]['IS_CHARGES_PAID'] == 1 ? 'Yes' : 'No';
        arry1.push(Object.assign({}, obj1));
        if (i == this.excelData.length - 1) {
          this._exportService.exportExcel(
            arry1,
            'Order Cancellation Charges  ' + (this.paymentStatus== "unpaid" ? "Unpaid" : "Paid")+ ' Customer Report ' +
