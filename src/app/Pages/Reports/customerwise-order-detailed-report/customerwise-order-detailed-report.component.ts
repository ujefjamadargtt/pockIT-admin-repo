...
@Component({
  ...
  styleUrls: ['./customerwise-order-detailed-report.component.css'],
})
export class CustomerwiseOrderDetailedReportComponent {
  constructor(
  ...
  formTitle = 'Customer Wise Order Details';
  searchText: string = '';
  pageIndex = 1;
  pageIndex1 = 1;
  pageSize = 10;
  ...
  onKeyup(event: KeyboardEvent): void {
    ...
    if (this.CustomerEmailText.length >= 3 && event.key === 'Enter') {
      ...
      this.CustomerEmailText.length == 0 &&
      event.key === 'Backspace'
    ) {
      this.search();
    ...
    if (this.CustomerMobileText.length >= 3 && event.key === 'Enter') {
      ...
      this.CustomerMobileText.length == 0 &&
      event.key === 'Backspace'
    ) {
      this.search();
    ...
    } else if (this.SubcategoryText.length == 0 && event.key === 'Backspace') {
      this.search();
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (this.sernameText !== '') {
      likeQuery +=
        (likeQuery ? ' AND ' : '') +
        `SERVICE_NAME LIKE '%${this.sernameText.trim()}%'`;
    }
    if (this.seramtText !== '') {
      likeQuery +=
        (likeQuery ? ' AND ' : '') +
    ...
    if (this.MONTH1) {
      this.filterQuery2 =
        ...
        this.datepipe.transform(this.MONTH1, 'yyyy') +
        "' AND MONTH(JOB_COMPLETED_DATETIME) = '" +
    ...
    if (exportInExcel == false) {
      this.api
        .getOrderwiseJobCardDetailedReport(
          ...
          this.sortKey,
          sort,
          likeQuery + this.filterQuery + this.filterQuery1 + this.filterQuery2
        )
        .subscribe(
          (data) => {
            if (data['code'] == 200) {
    ...
    if (this.excelData.length > 0) {
      for (var i = 0; i < this.excelData.length; i++) {
        ...
        obj1['Customer Name'] = this.excelData[i]['CUSTOMER_NAME']
          ...
          : '-';
        obj1['Email Id'] = this.excelData[i]['CUSTOMER_EMAIL']
          ? this.excelData[i]['CUSTOMER_EMAIL']
          : '-';
        obj1['Mobile No.'] = this.excelData[i]['CUSTOMER_MOBILE_NUMBER']
          ? this.excelData[i]['CUSTOMER_MOBILE_NUMBER']
          : '-';
        ...
        (data) => {
          if (data['code'] == 200) {
            if (this.pageIndex1 == 1) {
              ...
              this.CustomersData = [...this.CustomersData, ...data['data']];
            this.totalrecords = data['count'];
          } else {
            this.CustomersData = [];
          }
