...
export class InvoiceReportsComponent {
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (this.date && this.date.length === 2) {
      ...
      dateQuery += ` AND  DATE(INVOICE_DATE) BETWEEN '${start}' AND '${end}' `
    }
    ...
    if (this.templatenameText !== '') {
      likeQuery +=
        (likeQuery ? ' AND ' : '') +
        `INVOICE_URL LIKE '%${this.templatenameText.trim()}%'`;
      this.istemplatenameFilterApplied = true;
    } else {
      this.istemplatenameFilterApplied = false;
    }
    ...
    if (this.bodyText !== '') {
      likeQuery +=
        (likeQuery ? ' AND ' : '') + `CUSTOMER_NAME LIKE '%${this.bodyText.trim()}%'`;
      this.isbodyFilterApplied = true;
    } else {
      this.isbodyFilterApplied = false;
    }
    ...
    likeQuery = globalSearchQuery + ` AND TYPE = '${this.invoicetype}'` + (likeQuery ? ' AND ' + likeQuery : '');
    const finalDataList =
  ...
  ziploader: boolean = false;
  zipFile: any;
  zipUrl: any;
  getdataforexcel(reset: boolean = false, exportInExcel: boolean = false) {
    if (
      this.searchText.trim().length < 3 &&
      this.searchText.trim().length !== 0
    ) {
    ...
    this.ziploader = true;
    if (exportInExcel == false) {
      this.api.getInvoice(0, 0, this.sortKey, sort, likeQuery + this.filterQuery + dateQuery)
        .subscribe(
          (data) => {
            if (data['code'] == 200) {
              this.loadingRecords = false;
              const invoiceIds = data['data'].map((row: any) => row.ID);
              this.api.convertinvoicetozip(0, 0, "ID", "desc", likeQuery + this.filterQuery + dateQuery, invoiceIds, this.invoicetype).subscribe((res) => {
                if (res['code'] == 200) {
                  this.ziploader = false;
                  this.zipFile = res['zipFile'];
                  const a = this.api.retriveimgUrl + 'InvoiceZips' + '/' + this.zipFile;
                  this.zipUrl = a;
                  this.downloadZip();
                  this.message.success('Invoice Zip downloaded successfully.', '');
                } else {
                  this.ziploader = false;
                  this.message.error('Failed to create zip file.', '');
                }
              });
            } else if (data['code'] == 400) {
              this.loadingRecords = false;
