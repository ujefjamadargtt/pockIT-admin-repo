...
@Component({
  ...
  styleUrls: ['./sms-transaction-history-report.component.css'],
})
export class SmsTransactionHistoryReportComponent {
  filterGroups: any[] = [
    {
      operator: 'AND',
      conditions: [
    ...
    if (this.excelData.length > 0) {
      for (var i = 0; i < this.excelData.length; i++) {
        ...
        if (this.excelData[i]['STATUS'] == '1') {
          ...
          obj1['Status'] = 'In active';
        }
        arry1.push(Object.assign({}, obj1));
        if (i == this.excelData.length - 1) {
          this._exportService.exportExcel(
            arry1,
            'SMS Transaction History Report ' +
