...
@Component({
  selector: 'app-reschedulejobrequest',
  templateUrl: './reschedulejobrequest.component.html',
  styleUrls: ['./reschedulejobrequest.component.css'],
})
export class ReschedulejobrequestComponent {
  formTitle = 'Manage Job Reschedule Requests';
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  dataList: any = [];
  ...
  isSpinning = false;
  showcolumn = [
    { label: 'Order No. ', key: 'ORDER_NO', visible: true },
    { label: 'Job No ', key: 'JOB_CARD_NO', visible: true },
    ...
    { label: 'Rescheduled Date ', key: 'REQUESTED_DATE', visible: true },
    { label: 'Schedule Date ', key: 'SCHEDULED_DATE_TIME', visible: true },
  ...
  columns1: { label: string; value: string }[] = [
    ...
    { label: 'Job No. ', value: 'JOB_CARD_NO' },
  ...
  ngOnInit() {
    ...
    this.roleids = roleid;
    if (roleid == '9') {
      this.api.getVendorData(0, 0, '', '', ' AND USER_ID=' + this.USER_ID).subscribe(
        (dataaa1) => {
          if (dataaa1['code'] == 200) {
            if (dataaa1.count > 0) {
  ...
  search(reset: boolean = false) {
    ...
    if (this.orderrno !== '') {
      likeQuery +=
        (likeQuery ? ' AND ' : '') +
        `ORDER_NO LIKE '%${this.orderrno.trim()}%'`;
    }
      ...
      .subscribe(
        (data) => {
          if (data['status'] == 200) {
            ...
            this.loadingRecords = false;
            this.dataList = [];
            this.message.error('1Something Went Wrong ...', '');
          }
        },
        (err: HttpErrorResponse) => {
          this.loadingRecords = false;
          if (err.status === 0) {
            ...
            this.message.error('2Something Went Wrong.', '');
          }
        }
      );
  }
  ...
  onKeyup(event: KeyboardEvent, field: string): void {
    ...
    if (this.techname.length >= 3 && event.key === 'Enter' && field == 'TECH') {
      this.search();
      this.iscustnmFilterAppliedtech = true;
    } else if (
      ...
      this.search();
      this.iscustnmFilterAppliedtech = false;
    }
    if (
  ...
  appreorder(data: any, action: any) {
    ...
    if (
      ...
      var dataaa = {
        ...
        SCHEDULED_DATE_TIME: this.datepipe.transform(
          data.SCHEDULED_DATE_TIME,
          'yyyy-MM-dd'
        ),
        REQUESTED_DATE:
          this.datepipe.transform(data.REQUESTED_DATE, 'yyyy-MM-dd HH:mm') +
          ':00',
          ...
          if (successCode.status == 200) {
            if (this.action == 'A') {
              this.message.success(
                'The Reschedule request approved successfully.',
                ''
              );
            } else {
              this.message.success(
  ...
  getpendingjobsforedit(jobno: any) {
    this.api
      .getJobCardsForSchedular(
        ...
        '',
        '',
        " AND STATUS = 'P'  AND JOB_CARD_NO='" + jobno + "'"
      )
      .subscribe(
        (data) => {
          if (data['code'] == 200) {
            if (data['data'].length == 0) {
              this.message.error('No Pending job found', '');
            } else {
              var filterterritory = this.territoryData.filter(
                (x: any) => data['data'][0].TERRITORY_ID == x.ID
              );
