...
import { Ticket, Ticketgroup } from 'src/app/Support/Models/TicketingSystem';
...
export class TicketreportComponent implements OnInit {
  formTitle = 'Ticket Details Report';
  pageIndex = 1;
  pageSize = 10;
  totalRecords = 1;
  dataList: any = [];
  ...
  search(
    ...
    if (this.selectedDate1 != undefined && this.selectedDate2 != undefined) {
      dateFilter =
        " AND (DATE BETWEEN '" +
        this.datePipe.transform(this.selectedDate1, 'yyyy-MM-dd 00:00:00') +
        "' AND '" +
  ...
  applyFilter() {
    ...
    this.selectedDate2 = this.datePipe.transform(
      new Date(this.selectedDate2),
      'yyyy-MM-dd'
    );
    if (
      (this.selectedDate1 != null && this.selectedDate2 != null) ||
  ...
  convertInExcel() {
    ...
    for (var i = 0; i < this.status2.length; i++) {
      ...
      else if (this.status2[i] == "'C'") a = 'Closed';
      else if (this.status2[i] == "'S'") a = 'Assigned';
      else if (this.status2[i] == "'R'") a = 'Answered';
    ...
    for (var i = 0; i < this.dataListForExport.length; i++) {
      obj1['Ticket No.'] = this.dataListForExport[i]['TICKET_NO']
        ...
        : '-';
      obj1['Created On'] = this.dataListForExport[i]['DATE']
        ? this.datePipe.transform(
      ...
      obj1['Created By'] = this.dataListForExport[i]['CREATOR_EMPLOYEE_NAME']
        ...
        : '-';
      obj1['Department Name'] = this.dataListForExport[i]['DEPARTMENT_NAME']
        ? this.dataListForExport[i]['DEPARTMENT_NAME']
        : '-';
      obj1['Mobile No.'] = this.dataListForExport[i]['MOBILE_NO']
        ? this.dataListForExport[i]['MOBILE_NO']
        : '-';
      obj1['Email ID'] = this.dataListForExport[i]['EMAIL_ID']
        ? this.dataListForExport[i]['EMAIL_ID']
        : '-';
      ...
      if (i == this.dataListForExport.length - 1) {
        ...
        filters.push(
          'Date : ' +
          this.datePipe.transform(this.selectedDate1, 'dd/MM/yyyy ') +
          ' - ' +
          this.datePipe.transform(this.selectedDate2, 'dd/MM/yyyy ')
        );
        filters.push('Department(s): ' + this.getDepartmentToShow());
  ...
  getEmployeeToShow() {
    if (
      this.employeeNameToPrint == '' ||
      this.EMPLOYEE.length == this.employeelist.length
    )
      return 'All';
  ...
  onSelectOff1(event) {
    var a = this.employeelist.length;
    var b = this.employeelist.length - event.length;
    if ((a! = b)) {
      this.SELECT_ALL1 = false;
    } else {
