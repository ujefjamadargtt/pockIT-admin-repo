...
@Component({
  ...
  styleUrls: ['./stocks-by-unit-report.component.css'],
})
export class StocksByUnitReportComponent {
  filterGroups: any[] = [
    {
      operator: 'AND',
      conditions: [
  ...
  stocktext = '';
  actualunittext = '';
  stocksearchVisible = false;
  stockFilterApplied = false;
  actualUnitVisible = false;
  ...
  onKeyup(keys) {
    ...
    } else if (this.actualunittext.length == 0 && keys.key === 'Backspace') {
      this.search();
      this.actualUnitfilterApplied = false;
    }
    if (this.stocktext.length > 0 && keys.key === 'Enter') {
      this.search();
      this.stockFilterApplied = true;
    } else if (this.stocktext.length == 0 && keys.key === 'Backspace') {
      this.search();
    ...
    this.api.getWarehouses(0, 0, 'NAME', 'ASC', ' AND STATUS = 1').subscribe(
      (data: HttpResponse<any>) => {
        if (data['code'] == 200) {
          ...
          this.CategoryData = [];
          this.message.error('Failed To Get Warehouse Data', '');
        }
      },
      () => {
        this.message.error('Something Went Wrong', '');
  ...
  search(reset: boolean = false, exportInExcel: boolean = false) {
    ...
    if (this.stocktext !== '') {
      likeQuery +=
        (likeQuery ? ' AND ' : '') +
        `CURRENT_STOCK LIKE '%${this.stocktext.trim()}%'`;
      this.stockFilterApplied = true;
    } else {
      this.stockFilterApplied = false;
    }
    if (this.actualunittext !== '') {
      likeQuery +=
        (likeQuery ? ' AND ' : '') +
        `ACTUAL_UNIT_NAME LIKE '%${this.actualunittext.trim()}%'`;
      this.actualUnitfilterApplied = true;
    } else {
      this.actualUnitfilterApplied = false;
    }
  ...
  getWarehousesdata() {
    this.api;
    this.api
      .getWarehouses(0, 0, 'NAME', 'ASC', ' AND STATUS = 1')
  ...
  convertInExcel() {
    ...
    if (this.excelData.length > 0) {
      for (var i = 0; i < this.excelData.length; i++) {
        obj1['Current Stock'] = this.excelData[i]['CURRENT_STOCK']
          ? this.excelData[i]['CURRENT_STOCK']
          : '-';
        obj1['Unit'] = this.excelData[i]['ACTUAL_UNIT_NAME']
