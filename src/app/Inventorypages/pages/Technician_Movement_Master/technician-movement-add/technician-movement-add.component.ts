...
@Component({
  ...
  styleUrls: ['./technician-movement-add.component.css'],
})
export class TechnicianMovementAddComponent {
  radioValue = '';
  @Input()
  ...
  @Input()
  data: any = new TechnicianRequestMovement();
  ...
  getTechnician() {
    ...
    this.api
      .getTechnicianDataforStockMovement(
        ...
        '',
        'desc',
        ' AND TECHNICIAN_STATUS=1', wmanager
      )
      .subscribe((data) => {
        if (data['code'] == 200) {
    ...
    if (this.isOk) {
      ...
      if (this.data.ID) {
        ...
        const selectedWarehouse1111 = this.LoadGodownMain.find(
          (val: any) => val.ID === this.data.WAREHOUSE_ID
        );
        const techician = this.technicianData.find(
          (val: any) => val.ID === this.data.TECHNICIAN_ID
        );
        this.data.WAREHOUSE_NAME = selectedWarehouse1111
          ...
          (successCode) => {
            if (successCode['status'] == 200) {
              this.message.success(
                'Technician wise Stock Transfer Update Successfully',
                ''
              );
              ...
              this.message.error(
                'Technician wise Stock Transfer Updation failed',
                ''
              );
              this.isSpinning = false;
            }
  ...
  addData(addNew: boolean, form2: NgForm): void {
    ...
    if (existingIndex > -1) {
      ...
      const selectedVariant = this.data2.VARIANT_ID
        ...
        : null;
      const isVerient = selectedVariant ? 1 : 0;
      this.INNERTABLEDATA = {
        ...
        QUANTITY_PER_UNIT: selectedVariant
          ? selectedVariant.BASE_QUANTITY
          : this.ItemDetails.BASE_QUANTITY,
      };
            ...
            if (unitdata.code == 200) {
              ...
              if (unitdata['count'] > 0) {
                ...
                if (
                  ...
                  this.data2.STOCK_UNIT_ID = FiltUnit.UNIT_ID;
            ...
            if (unitdata.code == 200) {
              ...
              if (unitdata['count'] > 0) {
                ...
                if (
                  ...
                  this.data2.STOCK_UNIT_ID = FiltUnit.BASE_UNIT_ID;
