...
export class ListStockMovementRequestMasterComponent {
  ...
  add(): void {
    ...
    // this.api.stockMovementRequestnew(1, 1, 'ID', 'DESC', '').subscribe(
    //   (data) => {
    //     if (data['status'] == 200) {
      ...
      this.api.getWarehouses(0, 0, 'ID', 'desc', ' AND STATUS=1').subscribe(
        (data) => {
          if (data['code'] == 200) {
            ...
            // this.selectedWarehouses = this.warehouseList.map((warehouse) => warehouse.ID);
            // this.selectedDestinationWarehouses = this.warehouseList2.map((warehouse) => warehouse.ID);
