...
@Component({
  selector: 'app-ordercancelwise-detailed-report',
  templateUrl: './ordercancelwise-detailed-report.component.html',
  styleUrls: ['./ordercancelwise-detailed-report.component.css']
})
export class OrdercancelwiseDetailedReportComponent {
  constructor(
  ...
  onKeyupS(keys: KeyboardEvent) {
    if (this.searchText.length >= 3 && keys.key === 'Enter') {
      this.skipEmptyCheckedOnce = true;
      this.search(true);
    ...
    if (exportInExcel == false) {
      this.api
        .getOrderwiseJobCardDetailedReport(
          ...
          this.sortKey,
          sort,
          ' AND ORDER_ID = ' + this.orderId + likeQuery + this.filterQuery + " AND ORDER_STATUS NOT IN ('CA','CO','OR','PC')"
        )
        .subscribe(
          (data) => {
            if (data['code'] == 200) {
  ...
  skipEmptyCheckedOnce: boolean = false;
  ...
  openModal(data: any): void {
    this.selectedOrderId = this.orderdetailsdata['ORDER_ID'];
    this.selectedCustomerId = this.orderdetailsdata['CUSTOMER_ID'];
    this.selectedJobCardIds = data['JOB_CARD_NO'] ? [data['JOB_CARD_NO']] : [];
    this.techicianId = data['TECHNICIAN_ID'];
    this.modalTitle = data && data.JOB_CARD_NO
      ? `Do you want to cancel job ${data.JOB_CARD_NO} `
      : 'Do you want to cancel order ';
    if (data['JOB_CARD_STATUS'] === 'Cancelled' || data['JOB_CARD_STATUS'] === "Completed") {
  ...
  onReasonChange(value: any) {
    const reasonText =
      typeof value === "string"
        ? value.trim()
        : (value?.REASON || "").trim();
    this.showTextArea = reasonText.includes("Other");
    if (!this.showTextArea) {
      this.Reason = "";
    }
  }
  isOkLoading: boolean = false
  @ViewChild('ordercancelForm') form!: NgForm
  handleOrderOk(): void {
    if (!this.showCharges) {
      this.charges = 0;
    }
    if (this.showCharges === true) {
      if (!this.charges || this.charges === null || this.charges === undefined) {
        this.message.error("Please enter cancellation charges", '');
        return;
      }
    }
    ...
    let finalReason = "";
    if (this.showTextArea) {
      finalReason = this.Reason.trim();
    } else {
      finalReason = (this.selectedReason?.REASON || "").trim();
    }
    this.isOkLoading = true;
    this.api.cancelorderByAdmin(this.selectedOrderId,
      this.selectedCustomerId,
      finalReason,
      this.selectedJobCardIds,
      this.charges,
      this.techicianId).subscribe((data) => {
        if (data['code'] == 200) {
          this.message.success("Order cancelled sucessfully", '');
